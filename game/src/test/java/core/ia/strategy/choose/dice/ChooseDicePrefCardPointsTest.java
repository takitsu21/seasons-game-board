...
public class ChooseDicePrefCardPointsTest {
    private Config config = new Config();
    private GameController controller = new GameController();
    private Board board;
    private FacadeIA facadeIA;
    private ChooseDicePrefCardPoints chooseDicePrefCardPoints;
    ...
    void setUp() {
        ...
        chooseDicePrefCardPoints = new ChooseDicePrefCardPoints();
        ...
        when(face1.getEnergy()).thenReturn(new Energy[]{Energy.FIRE, Energy.FIRE, Energy.FIRE});
        when(face2.getEnergy()).thenReturn(null);
        when(face3.getEnergy()).thenReturn(null);
        when(face4.getEnergy()).thenReturn(new Energy[]{
                Energy.WIND, Energy.WIND
        });
    ...
    @Test
    void choose() {
        assertEquals(d4, chooseDicePrefCardPoints.choose(player));
    }
}
