...
public class ChooseDicePrefActivateTest {
    private Config config = new Config();
    private GameController controller = new GameController();
    private Board board;
    private FacadeIA facadeIA;
    private ChooseDicePrefActivate chooseDicePrefActivate;
    ...
    void setUp() {
        ...
        deck = new Deck();
        Card card1 = deck.findCard(23);
        Card card2 = deck.findCard(20);
        Card card3 = deck.findCard(15);
        Card card4 = deck.findCard(16);

        when(facadeIA.getCardInHand()).thenReturn(new ArrayList<>(List.of(
                card1, card2, card3, card4
        )));
        player.setFacadeIA(facadeIA);
        chooseDicePrefActivate = new ChooseDicePrefActivate();
        ...
        when(face3.getEnergy()).thenReturn(new Energy[]{Energy.FIRE});
        when(face4.getEnergy()).thenReturn(new Energy[]{
                Energy.WIND, Energy.FIRE, Energy.FIRE
        });
    ...
    @Test
    void choose() {
        assertEquals(d4, chooseDicePrefActivate.choose(player));
    }
