...
class PotionSavoirEffectTest {
    private Deck deck;
    private Player p1;
    private Inventory inventory1;
    private Card potionSavoir;
    ...
    public void setUp() {
        ...
        deck = new Deck();
        potionSavoir = deck.findCard(25);
    }

    @Test
    void useTestMaxEnergy() {
        inventory1.getHand().addCard(potionSavoir);
        ...
        assertTrue(inventory1.summonCard(potionSavoir, controller.getBoard()));
        assertEquals(0, inventory1.getEnergyStock().getEnergyStock().size());

        potionSavoir.use(controller.getBoard(), p1);
        assertEquals(5, inventory1.getEnergyStock().getEnergyStock().size());
    ...
    void useTest() {
        ...
        assertEquals(5, inventory1.getEnergyStock().getEnergyStock().size());
        potionSavoir.use(controller.getBoard(), p1);
        assertEquals(7, inventory1.getEnergyStock().getEnergyStock().size());

    }

    @Test
    void isMagic() {
        assertTrue(potionSavoir.isMagic());
    }

    @Test
    void isActivable() {
        assertTrue(potionSavoir.isActivable());
    }

    @Test
    void isActivatedTest() {
        assertFalse(potionSavoir.isActivated());
    }

    @Test
    void effectTypeTest() {
        assertEquals(EffectType.NONE, potionSavoir.getEffectType());
    }
}
